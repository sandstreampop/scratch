 ...
 * stacking context below .tbtn (z4). All panel events stopPropagation so the
 * document-level touch handlers in 20_input.js never see them (this is ALSO what
 * keeps the sliders draggable: input's document touchmove preventDefaults).
 *
...
/* Block every input event at the panel/button boundary so 20_input.js document/
 * window handlers (which preventDefault touchmove!) never see menu touches. */
function shield(el){
  var i;
...
/* iOS-safe tap: touchend preventDefault kills the synthetic click (no double
 * fire); the click handler is the desktop path. */
function tap(el, fn){
  el.addEventListener('touchstart', function(e){ eat(e); el.classList.add('dn'); }, {passive:false});
  el.addEventListener('touchend', function(e){ eat(e); el.classList.remove('dn'); fn(); }, {passive:false});
  el.addEventListener('touchcancel', function(e){ stopOnly(e); el.classList.remove('dn'); }, {passive:false});
  ...
  el.addEventListener('click', function(e){ eat(e); fn(); });
}

/* ---------------- rig bridge (defensive vs mid-build siblings) ---------------- */
function rigSetType(t){
  var R = JK.Rig;
  if (R && R.setType){ R.setType(t); return true; }
  if (R && R.player && R.player.setType){ R.player.setType(t); return true; }
  return false;
}
function rigSetSaber(f){
  var R = JK.Rig;
  if (R && R.setSaber){ R.setSaber(f); return true; }
  if (R && R.player && R.player.setSaber){ R.player.setSaber(f); return true; }
  return false;
}
 ...
 * 'input' path. Rect math only runs during menu interaction — not per frame. */
function bindSlider(inp, idx){
  ...
  inp.addEventListener('touchstart', function(e){ eat(e); fromX(e.changedTouches[0].clientX); }, {passive:false});
  inp.addEventListener('touchmove', function(e){ eat(e); fromX(e.changedTouches[0].clientX); }, {passive:false});
  inp.addEventListener('touchend', function(e){ eat(e); applyColor(true); }, {passive:false});
  inp.addEventListener('touchcancel', function(e){ stopOnly(e); applyColor(true); }, {passive:false});
